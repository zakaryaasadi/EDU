...
$(document).ready(function () {
    for (i = 0; i < values.subjects.length; i++) {
        var s = values.subjects[i];
        var id = '#' + s.subject_name + s.subject_id;
        $(id).prop('checked', true);
        $('#' + s.Class.class_name + s.Class.class_id).prop('checked', true);
        myObj.fields.subjects.push(s.subject_id);
    }
});
...
var myObj = new Vue({
    ...
    data: {
        ...
        fields: {
            ...
            date_of_birth: values.date_of_birth, gender_id: values.gender_id,
            joining_date: values.joining_date, phone: values.phone, address: values.address, subjects: []
        },
        e: {
    ...
    if (event.target.checked) {
        for (i = 0; i < c.subjects.length; i++) {
            ...
            if (!subject.is_selected) {
                myObj.fields.subjects.push(subject.subject_id);
        ...
        for (i = 0; i < c.subjects.length; i++) {
            ...
            const index = myObj.fields.subjects.indexOf(subject.subject_id);
            if (index > -1) {
                myObj.fields.subjects.splice(index, 1);
            }
        }
    }
}
